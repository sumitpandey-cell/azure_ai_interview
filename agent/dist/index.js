    ...
    entry: async (ctx) => {
        ...
        try {
            ...
            try {
                ...
                throw new AgentError(`Failed to connect to LiveKit: ${error instanceof Error ? error.message : String(error)}`, "LIVEKIT_CONNECTION_ERROR", false);
            }
            ...
            try {
                ...
                throw new AgentError(`Failed to wait for participant: ${error instanceof Error ? error.message : String(error)}`, "PARTICIPANT_WAIT_ERROR", false);
            }
            // Create Audio Source for sending Azure audio → LiveKit
            let azureAudioSource;
            try {
                azureAudioSource = new AudioSource(24000, 1);
            }
            catch (error) {
                throw new AgentError(`Failed to create audio source: ${error instanceof Error ? error.message : String(error)}`, "AUDIO_SOURCE_ERROR", false);
            }
            ...
            try {
                const aiTrack = await ctx.room.localParticipant.publishTrack(LocalAudioTrack.createAudioTrack("ai-audio", azureAudioSource), new TrackPublishOptions({ source: TrackSource.SOURCE_UNKNOWN }));
            }
            catch (error) {
                throw new AgentError(`Failed to publish audio track: ${error instanceof Error ? error.message : String(error)}`, "TRACK_PUBLISH_ERROR", false);
            }
            ...
            try {
                ...
                throw new AgentError(`Failed to create WebSocket connection: ${error instanceof Error ? error.message : String(error)}`, "WEBSOCKET_CREATE_ERROR", true);
            }
            ws.on("open", () => {
                try {
            ...
            const sendTranscriptToRoom = (speaker, transcript, isComplete = true, timestamp) => {
                try {
                    ...
                    ctx.room.localParticipant?.publishData(new TextEncoder().encode(transcriptData), { reliable: true });
                }
                catch (error) {
            ...
            const startPlaybackLoop = () => {
                if (playbackInterval || isShuttingDown)
                    return;
            ...
            const sendFrame = (chunk) => {
                try {
                    const int16Array = new Int16Array(chunk.length / 2);
                    for (let i = 0; i < int16Array.length; i++) {
                        int16Array[i] = chunk.readInt16LE(i * 2);
                    }
                    const frame = new AudioFrame(int16Array, SAMPLE_RATE, CHANNELS, int16Array.length / CHANNELS);
