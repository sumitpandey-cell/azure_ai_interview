...
class RoadmapService {
     ...
     */
    async checkPaymentEligibility(userId: string, supabaseClient?: SupabaseClient<Database>): Promise<PaymentEligibility> {
        try {
            const client = supabaseClient || supabase;
            // Count total roadmaps generated by user (only those that are completed or free)
            ...
            return {
                requiresPayment: roadmapCount >= 1, // 1st one is free
                roadmapCount,
    ...
    async generateRoadmap(
        ...
        try {
            ...
            if (!force) {
                ...
                if (existing) {
                    // Safely access roadmap_data as a potential object with a status property
                    const roadmapData = existing.roadmap_data as { status?: string } | null;
                    if (roadmapData?.status === 'generating') {
            ...
            // 5. Check eligibility (minimum 1 completed interview for personalized, or allow if manual info provided)
            // If they don't have interviews BUT provided domain/role, we allow it.
            if (history.completed.length < 1 && !domain && !role) {
                throw new Error('Minimum 1 completed interview required or provide target domain/role');
            }
            ...
            // 6. Build AI prompt
            const prompt = this.buildAnalysisPrompt(history, domain, role, level);
            ...
            // 9. Determine overall level (use provided level or calculate from history)
            const overallLevel = (level as 'Beginner' | 'Intermediate' | 'Advanced' | undefined) || this.calculateOverallLevel(history);

            // 11c. Payment info
            const isFree = !eligibility.requiresPayment;
            const paymentStatus = isFree ? 'free' : (paymentId ? 'completed' : 'pending');
            ...
            if (placeholderId && (error as { code?: string })?.code !== '42501') {
                try {
                    await client
                        .from('learning_roadmaps')
                        .delete()
     ...
     */
    private buildAnalysisPrompt(history: Record<string, unknown>, domain?: string, role?: string, level?: string): string {
        const targetContext = domain || role ? `
## Target Goal:
...
` : '';
        ...
        const historyContext = (history as { completed?: unknown[] }).completed?.length ? `
## Interview History Data:
${JSON.stringify(history, null, 2)}
` : '## Interview History Data: No previous interview data available.';

        return `You are an expert career coach and technical interview mentor. Your specialty is creating roadmaps for INTERVIEW MASTERY.
        
${targetContext}
${historyContext}

## Your Task:
Create a personalized learning roadmap specifically focused on helping the candidate master interviews for their target role. If history data is available, prioritize fixing identified weaknesses. If no history is available, build a comprehensive guide based on industry standards for the specified Domain and Role.

Generate the roadmap in JSON format with the following structure:
...
Return ONLY valid JSON, no markdown formatting, no code blocks, just the raw JSON object.`;
     ...
     */
    async getActiveRoadmap(userId: string, supabaseClient?: SupabaseClient<Database>) {
        try {
            const client = supabaseClient || supabase;
            ...
            if (data && (data.roadmap_data as unknown as Record<string, string>)?.status === 'generating') {
     ...
     */
    async refreshRoadmap(userId: string, paymentId?: string, supabaseClient?: SupabaseClient<Database>, domain?: string, role?: string, level?: string) {
        return this.generateRoadmap(userId, paymentId, supabaseClient, true, domain, role, level);
    }
        ...
        try {
            ...
            const { data, error } = await client
                ...
                .insert({
                    ...
                    user_id: userId,
                    item_type: itemType as "phase" | "milestone" | "goal" | "interview" | "resource" | null,
                    milestone_id: itemId,
     ...
     */
    async getProgress(userId: string, roadmapId: string, supabaseClient?: SupabaseClient<Database>) {
        try {
            const client = supabaseClient || supabase;
     ...
     */
    private calculateOverallLevel(history: Record<string, unknown>): 'Beginner' | 'Intermediate' | 'Advanced' {
        const stats = (history as { stats?: { average_score?: number } }).stats;
     ...
     */
    private isExpired(roadmap: { expires_at: string }): boolean {
